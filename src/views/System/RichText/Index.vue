<template>
  <div class="bf-content">
      ...
      <table-search :current-page="form.page" :page-size="form.size" :total="form.totals" @size-change="handleSizeChange" :isAdvancedSearch="false"
        ...
        <el-form :model="form" label-width="70px" class="bf-search-formItem">
          <div class="flex justify-between items-center">
            <div class="bf-table-label">富文本管理</div>
            <el-row :gutter="10" justify="end">
              ...
              <el-col :md="24" :lg="7">
                <el-form-item label="业务类型">
                  <el-input v-model="form.Type" placeholder="业务类型" />
                </el-form-item>
              </el-col>
              <el-col :md="24" :lg="6">
                <el-form-item label="编码">
                  <el-select v-model="form.Status" placeholder="编码" clearable>
          ...
          <el-table :data="data.tableData" class="w-full">
            ...
            <el-table-column prop="Code" label="编码">
              <template #default="scope">
                <div class="bf-font-blue-color">{{scope.row.Code}}</div>
              </template>
            </el-table-column>
    ...
    <!-- 富文本表单 -->
    <RichTextForm v-if="data.indexWindow.show" @closeWindow="closeWindow" :Id="data.indexWindow.Id"></RichTextForm>
  </div>
...
import RichTextForm from './_Components/RichTextForm.vue'
import {
...
import { GetDeleteHRRichText, GetHRRichTextPage } from '/@/api/system/richText'
const store = useCommonStore() //记得加这一句
// 列表数据请求
const renderTableList = async (form: any, data: any) => {
  await GetHRRichTextPage(form).then((res) => {
    if (res.code == 1) {
...
const window = (form: any, data: any): any => {
  ...
  const openMessage = (val: number) => {
    ElMessageBox.confirm('是否删除此条信息?', '提示', {
      ...
      GetDeleteHRRichText(val).then((res) => {
        if (res.code == 1) {
          form.page = 1
