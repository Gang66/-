<template>
  <OpenWindow :boxStyle="boxStyle" :isShow="true" @closeWindow="closeWindow" title="个性化菜单">
    <div class="bf-window-padding">
      ...
      <el-tree :allow-drop="allowDrop" :allow-drag="allowDrag" :data="data.dataSource" @node-click="nodeClick" draggable default-expand-all highlight-current :expand-on-click-node="false" node-key="id" id="MenuTree" ref="treeRef">
...
import { GetPostMenuCustomList, GetUserMenuCustomList, AddPostMenuGroupCustomSave, EditDeletePostMenuGroupCustomSave, EditPostMenuCustomSave, AddUserMenuGroupCustomSave, EditDeleteUserMenuGroupCustomSave, EditUserMenuCustomSave } from '/@/api/menu/menu'
...
const getUserMenuCustomTree = async (data: any): void => {
  await GetUserMenuCustomList(data.UserId).then((res) => {
    if (res.code == 1) {
...
const getPostMenuCustomTree = async (data: any): void => {
  await GetPostMenuCustomList(data.PostId).then((res) => {
    if (res.code == 1) {
      data.dataSource = NoCustomMenu(res.data, 0)
    } else
      ElNotification({
...
const submitData = async (data: any, closeWindow): void => {
  if (data.PostId != 0) {
    await EditPostMenuCustomSave(data.dataSource).then((res) => {
      if (res.code == 1) {
    ...
    await EditUserMenuCustomSave(data.dataSource).then((res) => {
      if (res.code == 1) {
        ElMessage.success('操作成功')
        closeWindow()
      } else
...
const addGroup = async (data, val): void => {
  if (data.PostId != 0) {
    ...
    await AddPostMenuGroupCustomSave(p).then((res) => {
      if (res.code == 1) {
        let d: DataForTree = {
          Id: res.data,
          ParentId: 0,
    ...
    await AddUserMenuGroupCustomSave(p).then((res) => {
      if (res.code == 1) {
        let d: DataForTree = {
          Id: res.data,
          ParentId: 0,
...
const delGroup = async (treeRef, data): void => {
  if (data.PostId != 0) {
    await EditDeletePostMenuGroupCustomSave(data.DelTreeNode.data.Id).then((res) => {
      if (res.code == 1) {
    ...
    await EditDeleteUserMenuGroupCustomSave(data.DelTreeNode.data.Id).then((res) => {
      if (res.code == 1) {
        treeRef.remove(data.DelTreeNode)
        data.DelTreeNode = ''
      } else
