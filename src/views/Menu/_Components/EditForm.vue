	...
	<OpenWindow :boxStyle="boxStyle" :isShow="true" @closeWindow="closeWindow('close')" title="菜单管理">
		<el-form ref="ruleFormRef" :model="data.ruleForm" :rules="rules" label-width="80px" class="bf-window-padding"
			...
			<el-form-item label="选择父级"  prop="ParentId">
				<el-tree-select v-model="data.ruleForm.ParentId" placeholder="请选择父级菜单" @change="SelParentIdChange" :data='data.ParentSelectData' check-strictly style="width: 100%;" :props='data.TreeSelectProp'>
				</el-tree-select >
			</el-form-item>
			<el-form-item label="页面图标" prop="Icon">
				<el-select v-model="data.ruleForm.Icon" placeholder="请选择页面图标" style="width: 100%;">
					<el-option v-for="item in ElIconsData" :key="`ElIcon${item.name}`" :value="`ElIcon${item.name}`">
						...
						<span
							style="float: right;color: var(--el-text-color-secondary);font-size: 13px;">ElIcon{{ item.name }}</span>
					</el-option>
				</el-select>
			</el-form-item>
			...
			<el-form-item label="页面指令" prop="Permission" v-if="data.ruleForm.ParentId!=0">
				<template v-for="tag in data.ruleForm.Permission" :key="tag.Code">
					...
					size="large"
					closable
					@close="btn_DelPermission(tag)"
					@click="btn_EditPermissionWindow(tag)"
					>
					{{ tag.Remark }}
					</el-tag>
				  </template>
		...
		const submitForm = async (formEl: FormInstance | undefined) => {
			...
			await formEl.validate((valid, fields) => {
				if (valid) {
					...
					if(arr.ParentId==0&& arr.Component!='School')
						arr.Component="Layout";
					
					//判断是否是编辑
					if(data.IsEdit)
						EditMenuSave(arr).then((res) => {
							if (res.code == 1) {
								ElMessage({
									message: res.message,
									type: "success",
		...
		const SelParentIdChange=(val:any)=>{
			...
			try{
				...
				if(d.children&&d.children.length>0)
					data.ruleForm.OrderBy=d.children[d.children.length-1].OrderBy+1
				else
		...
		setup(props, context: SetupContext) {
			...
			onMounted(() => {
				...
				if(props.ParentSelectData.length>0){
					...
					data.ParentSelectData=[{"Id":0,"Title":"根目录"}]
				//获取本次编辑的数据
