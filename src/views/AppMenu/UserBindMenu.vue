    ...
    <div class="bf-center-content">
      <el-row>
        <el-col :md='24' :lg="4">
          <el-row :gutter='5'>
            <el-col :span='16'>
              <el-input v-model="data.seachUserVal" size="small" placeholder="搜索" @change="seachUserValueChange" clearable />
            </el-col>
            <el-col :span='8'>
          ...
          <el-row class='treeDiv'>
            <el-tree :data="data.seachUserTree.length==0?data.TreeUserData:data.seachUserTree" :props="data.defaultProps" @node-click="handleTreeNodeClick" style="width: 100%;" />
          </el-row>
        </el-col>
          ...
          <div class="bf-table-content">
            <el-table :data="data.tableTreeData" class="w-full" row-key='Id' default-expand-all>
              ...
              <el-table-column prop="Permission" label="指令授权" align="center">
                <template #default="scope">
                  <el-checkbox v-for='item in scope.row.Permission' v-model="item.IsGet" :label="item.Remark" @change="CheckPermissionChange($event,item.Id,scope.row)"
                    size="large" />
                </template>
              </el-table-column>
...
import { GetUserList, GetAppMenulistForUserList, EditAppUserBindMenuSave } from '/@/api/menu/menu'
...
const GetMenuData = async (data: any) => {
  ...
  const OtherMenuListToTree = (d: Array<MenuDataToList>, pid: number): Array<MenuDataToTree> => {
    ...
    d.forEach((item) => {
      if (item.ParentId == pid || d.findIndex((v) => v.Id == item.ParentId) > -1) return
      other.children.push(item)
    })
    if (other.children.length > 0) d2.push(other)
    return d2
  }

  //根据用户ID和搜索条件获取菜单绑定信息
  const d: GetMenuListParam = { UserId: data.EditUserId, MenuName: data.seachMenuNameVal, MenuPath: data.seachMenuPathVal }
  await GetAppMenulistForUserList(d).then((res) => {
    if (res.code == 1) {
...
const submitMenuChange = async (data: EditMenuData, item: MenuDataToTree) => {
  await EditAppUserBindMenuSave(data).then((res) => {
    if (res.code == 1) {
...
const tableOperation = (data: any) => {
  ...
  const btn_TableSearch = (): void => {
    if (data.EditUserId != 0) GetMenuData(data)
    else ElMessage.warning('请先选择人员')
  }
  //菜单授权
