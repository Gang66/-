    ...
    <div class="bf-center-content">
      <el-row>
        <el-col :md='24' :lg="4">
          <el-row :gutter='5'>
            <el-col :span='16'>
              <el-input v-model="data.seachPostVal" size="small" placeholder="搜索" @change="seachPostValueChange" clearable />
            </el-col>
            <el-col :span='8'>
          ...
          <el-row class='treeDiv'>
            <el-tree :data="data.seachPostData.length==0?data.PostData:data.seachPostData" :props="data.defaultProps" @node-click="handleTreeNodeClick" style="width: 100%;" />
          </el-row>
        </el-col>
...
import { GetPostList, GetAppMenulistForPostList, EditAppPostBindMenuSave } from '/@/api/menu/menu'
...
const GetMenuData = async (data: any) => {
  ...
  const OtherMenuListToTree = (mneuData: Array<MenuDataToList>, pid: number): Array<MenuDataToTree> => {
    ...
    mneuData.forEach((item) => {
      if (item.ParentId == pid || mneuData.findIndex((v) => v.Id == item.ParentId) > -1) return
  ...
  await GetAppMenulistForPostList(d).then((res) => {
    if (res.code == 1) {
...
const submitMenuChange = async (editData: EditMenuData, item: MenuDataToTree) => {
  await EditAppPostBindMenuSave(editData).then((res) => {
    if (res.code == 1) {
      ...
      if (editData.PermissionId == 0) {
        ...
        //如果是新增指令授权则同时授权页面
        if (editData.State) item.IsGet = true
        //更改指令状态
...
const tableOperation = (data: any) => {
  ...
  const btn_TableSearch = (): void => {
    if (data.EditPostId != 0) GetMenuData(data)
    else ElMessage.warning('请先选择岗位')
  }
  //菜单授权
