  ...
  <OpenWindow :boxStyle="boxStyle" :isShow="true" @closeWindow="closeWindow('close')">
    <div class="tech_inquiry_assignorder m-auto">
      <div class="flex flex-wrap max-h-96" style="overflow: auto">
        <template v-for="(item, index) in data.AccounterUserList" :key="index">
...
import { GetAccounterUserList } from '/@/api/tech/EnquiryData'
...
export default defineComponent({
  ...
  setup(props, context: SetupContext) {
    ...
    // 获取核算用户列表
    const getAccounterUserList = async (data: any) => {
      await GetAccounterUserList(data.id)
        .then((res) => {
          if (res.code == 1) {
            data.AccounterUserList = res.data || []
            console.log(res)
          } else {
