  ...
  <div class="tech_customedemand_index">
    ...
    <el-row :gutter="20">
      ...
      <el-col :span="6" v-show="!getSetting.showLayoutRight">
        <!-- 右侧 -->
        <CustomeDemandRight v-if="data.CustomeDemandRightShow"></CustomeDemandRight>
      </el-col>
    </el-row>
    <!--  分配厂商 (供应商库 )-->
...
||||||| .r18669
=======
      CustomeDemandRightShow: true, // 右侧组件是否展示
>>>>>>> .r19058
      // 表格数据
      tableData: [],
      number: 0,
...
||||||| .r18669
    const AddAudit = async () => {
      await GetSetEnquiryOrderAccounterAuditSave(form.EnquiryDataId).then(res => {
...
=======
    const AddAudit = async () => {
      await GetSetEnquiryOrderAccounterAuditSave(form.EnquiryDataId).then(res => {
        res.code == 1 ? ElMessage.success(res.message) : ElMessage.error(res.message)
        if(res.code == 1){
          data.CustomeDemandRightShow = false
          setTimeout(() => {
            data.CustomeDemandRightShow = true
          })
        }
      })
>>>>>>> .r19058
