    ...
    <div class="right grid-content ep-bg-purple mt-5 mb-0 ml-px">
      ...
      <div class="bg-white rounded relative tech_kehu">
        <div v-if="data.isMyOrder">
          <el-tabs v-model="activeName" class="demo-tabs">
            ...
            <el-tab-pane label="客户信息" name="first">
              <div class="w-11/12 m-auto">
                <div class="h-16 clearfix">
                  <div class="float-left h-16 w-16 bg-slate-100 rounded-md">
                    <img class="h-16 w-16 rounded-full" src="../../../../assets/img/tech/header.jpg" alt="" />
                  </div>
                ...
                <div class="h-16 bg-blue-50 my-3.5 rounded">
                  <div class="flex m-3 text-center mt-0">
                    ...
                    <div class="w-2/6 mt-2">
                      ...
                      <div>
                        <span class="text-sm font-semibold text-green-400">{{ data.CustomerOrderStatistics.EffectiveOrderCount }}</span>
                      </div>
                    </div>
                    <div class="w-2/6 mt-2">
                      <div style="position: relative">
                      ...
                      <div>
                        <span class="text-sm font-semibold text-yellow-500">{{ data.CustomerOrderStatistics.AllOrderCount }}</span>
                      </div>
                    </div>
                    <div class="w-2/6 mt-2">
                      <div style="position: relative">
                      ...
                      <div>
                        <span class="text-sm font-semibold text-blue-400">{{ getOrderStatus(data.orderInfo.OrderState) || '-' }}</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div class="absolute top-2 right-4" v-if="seek == 1" @click="support()"><i class="iconfont icon-dingdan6 w-10 h-10 mt-5 text-blue-500 text-sm" /><span class="text-sm text-blue-500 ml-1 cursor-pointer">寻求支援</span></div>
        </div>
        <div class="text-center pb-6" v-else>
          <svg-icon icon-class="svg-tech_dingdan" style="font-size: 13rem; margin: auto" />
          <span class="text-red-500">恭喜你，暂无需技术处理的订单！</span>
        </div>
      </div>
      <!-- bottom -->
      <div class="h-20 mt-3 bg-white rounded">
        <div class="h-12 mt-3 bg-blue-50 rounded pt-2.5 pl-4 pr-4">
          ...
          <div class="float-right -ml-2">
            <el-button type="primary" size="small" @click="searchEvent()"> <i class="iconfont icon-huntfor mr-2"></i> 查询</el-button>
          </div>
        </div>
        <div v-if="data.isMyOrder">
          <el-table
    ...
    <el-dialog v-model="data.dialogVisible" title="Tips" width="30%" :before-close="handleClose">
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="data.dialogVisible = false">Confirm</el-button>
        </span>
      </template>
    </el-dialog>
    <!-- 寻求支援 -->
    <SeekSupport title="寻求支援" v-if="seekSupportOrderOpenData.visible" :FormData="seekSupportOrderOpenData.formData" @closeWindow="closeWindowSeekSupportOrder($event)" />
...
import { GetOrderTechnicistTreatmList } from '/@/api/tech/EnquiryData'
...
import SeekSupport from './SeekSupport.vue'
...
// 获取技术订单处理列表
const getOrderTechnicistTreatmList = async (form: any, data: any) => {
  await GetOrderTechnicistTreatmList(form)
    .then(res => {
      if (res.code == 1) {
        var _list = res.data || []
...
export default defineComponent({
  ...
  setup() {
    ...
    const seek = values //是否显示寻求支援
    // 查询
    const searchEvent = () => {
      getOrderTechnicistTreatmList(form, data)
      console.log('查询')
    }
    ...
    // 寻求支援弹框
    const SeekSupportOrderOpen = (data: any) => {
      let seekSupportOrderOpenData: any = reactive({
        visible: false,
        formData: {}
      })
      //打开弹窗
      const openWindowSeekSupportOrder = (row: any) => {
        seekSupportOrderOpenData.formData = {
          Id: row
        }
        console.log(row)
        seekSupportOrderOpenData.visible = true
      }

      //关闭弹窗
      const closeWindowSeekSupportOrder = (type: string) => {
        seekSupportOrderOpenData.visible = false
        if (type == 'submit') data.getData(data.data)
      }

      // 寻求支援
      const support = () => {
        ElMessageBox.confirm('确定寻求总经理支援？', '寻求支援', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        })
          .then(() => {
            openWindowSeekSupportOrder({ Id: 22 })
            console.log(data)
          })
          .catch(() => {
            // ElMessage({
      ...
      return {
        seekSupportOrderOpenData,
        openWindowSeekSupportOrder,
        closeWindowSeekSupportOrder,
        support
      }
    }
    ...
    return {
      ...
      getFreight,
      downFiles,
      ...SeekSupportOrderOpen({ data, getData }),
      handleClose,
      getTextLength,
      ...FileOpenWindow,
      openFile
