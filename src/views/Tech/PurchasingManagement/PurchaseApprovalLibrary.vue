  ...
  <div class="Tec_ContractApproval">
    ...
    <div class="clearfix pt-1 pr-4 pb-1 pl-0 bg-gray-50 border-b border-gray-200 mb-0 rounded-t-md">
      <div class="float-left pt-1">
        <span class="ml-4 text-sm">采购核准审批库</span>
      </div>
    </div>
          ...
          <div class="bf-table-content">
            <el-table :data="data.tableData" stripe @selection-change="handleSelectionChange">
              ...
              <el-table-column prop="WorkAge" label="采购比例" align="center" width="80">
                <template #default="scope">
                  <span class="text-green-400" v-if="scope.row.PurchaseRate>2">{{ scope.row.PurchaseRate }}</span>
                  <span class="text-red-400" v-else>{{ scope.row.PurchaseRate }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="WorkAge" label="分组数量" align="center" width="80">
                <template #default="scope">
                  <span class="text-green-400" v-if="scope.row.GroupConfirmCount == scope.row.GroupConfirmCount && scope.row.GroupConfirmCount >0">{{ scope.row.GroupConfirmCount }} / {{ scope.row.GroupCount }}</span>
                  <span class="text-red-400" v-else>{{ scope.row.GroupConfirmCount }} / {{ scope.row.GroupCount }}</span>
                </template>
              </el-table-column>
    ...
    <!-- 采购清单 -->
    <!-- <PurchaseReservation title="阜新睿光氟化学有限公司 - 分项清单" v-if="purchaseReservationOpenData.visible" :FormData="purchaseReservationOpenData.formData" @closeWindow="closeWindowPurchaseReservation($event)" /> -->
    <!-- 采购清单 -->
...
import { GetMaterialBillVerifyPage } from '/@/api/tech/Material'
...
const renderTableList = async (form: any, data: any) => {
  ...
  await GetMaterialBillVerifyPage(form).then((res) => {
...
export default defineComponent({
  ...
  setup() {
    ...
    // // 采购清单
    // const purchaseReservationOpen = (data: any) => {
    //   let purchaseReservationOpenData: any = reactive({
    //     visible: false,
    //     formData: {}
    //   })

    //   //打开弹窗
    //   const openWindowPurchaseReservation = (val: any) => {
    //     purchaseReservationOpenData.formData = {}
    //     purchaseReservationOpenData.visible = true
    //   }

    //   //关闭弹窗
    //   const closeWindowPurchaseReservation = (type: string) => {
    //     purchaseReservationOpenData.visible = false
    //   }

    //   return {
    //     purchaseReservationOpenData,
    //     openWindowPurchaseReservation,
    //     closeWindowPurchaseReservation
    //   }
    // }
    // 采购清单
    ...
    return {
      ...
      ...purchaseListOpen({ data, getData }),
      // ...purchaseReservationOpen({ data })
    }
  }
})
</script>
