<template>
  <OpenWindow :boxStyle="boxStyle" :isShow="true" @closeWindow="closeWindow('close')">
    <el-form ref="ruleFormRef" :model="data.ruleForm" :rules="rules" label-width="80px" class="bf-window-padding" hide-required-asterisk>
      <el-form-item label="螺栓" prop="BasicType">
        <el-select v-model="data.ruleForm.BasicType" class="w-full" placeholder="请选择螺栓">
      ...
      <el-form-item label="螺栓规格" prop="Version">
        <el-input v-model="data.ruleForm.Version" placeholder="请输入螺栓规格" />
      </el-form-item>
      <el-form-item label="数量" prop="Quantity">
        <el-input-number v-model="data.ruleForm.Quantity" :min="1" :max="999" placeholder="请输入数量" />
      </el-form-item>
      <el-form-item label="等级" prop="GradeInfo">
        <el-select v-model="data.ruleForm.GradeInfo" class="w-full" placeholder="请选择等级">
          <el-option v-for="item in data.gradeList" :key="item.ID" :label="item.label" :value="item.ID" />
        </el-select>
      </el-form-item>
      <el-form-item>
...
export default defineComponent({
  ...
  setup(props, context: SetupContext) {
    ...
    const rules = reactive<FormRules>({
      ...
      GradeInfo: [{ required: true, message: '请选择等级', trigger: 'changge' }]
    })
    // 表单重置
    ...
    onMounted(() => {
      ...
      data.gradeList = props.gradeList
