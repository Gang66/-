      ...
      <div class="h-14 bg-blue-50">
        <div class="p-2 pl-6">
          ...
          <div class="float-right">
            ...
            <el-button type="primary" size="small" @click="openWindowApprovalStatus()">技术处理</el-button>
            <el-button type="primary" size="small" @click="openWindowAccountingRecords()">核算处理</el-button>
            <el-button type="primary" size="small" @click="openWindowHechnicalMarkup()">加价记录</el-button>
          </div>
        </div>
      </div>
      ...
      <div class="h-72 p-5">
        ...
        <div class="OrderAnalysis_right h-52 float-right">
          <div class="OrderAnalysis_right h-14 bg-slate-100 text-xs inline-block p-4 relative rounded-lg">
            <el-tag class="ml-2" type="danger">报价等级 {{ data.orderInfo.Grade }}</el-tag>
          ...
          <div class="h-20 bg-red-100 flex relative rounded-lg mt-2">
            <div class="w-1/4 h-20 mr-2 text-center pt-6">
              <p class="text-red-400" v-if="data.orderInfo.Grade === 'A'">优先订单</p>
              <p class="text-blue-400" v-else-if="data.orderInfo.Grade === 'B'">重点订单</p>
              <p class="text-green-400" v-else>普通订单</p>
            </div>
            <div class="w-1/4 h-20 mr-2 text-center text-sm">
              <p class="text-gray-400 mt-2">询价阶段</p>
              <p class="m-1">{{ data.orderInfo.IS_State }} {{ data.orderInfo.IS_Rate }}%</p>
              <el-progress :show-text="false" :percentage="data.orderInfo.IS_Rate" />
              <!-- <el-progress :percentage="data.orderInfo.IS_Rate" :status="data.orderInfo.IS_Rate >= 100 ? 'success' : 'warning'" /> -->
            </div>
            <div class="w-1/4 h-20 mr-2 text-center text-sm">
              <p class="text-gray-400 mt-2">询价性质</p>
              <p class="m-1">{{ data.orderInfo.IN_State }} {{ data.orderInfo.IN_Rate }}%</p>
              <el-progress :show-text="false" status="warning" :percentage="data.orderInfo.IN_Rate" />
              <!-- <el-progress :percentage="data.orderInfo.IN_Rate" :status="data.orderInfo.IN_Rate >= 100 ? 'success' : 'warning'" /> -->
            </div>
            <div class="w-1/4 h-20 text-center text-sm">
              <p class="text-gray-400 mt-2">询价数量</p>
              <p class="m-1">{{ data.orderInfo.IQ_State }} {{ data.orderInfo.IQ_Rate }}%</p>
              <el-progress :show-text="false" status="success" :percentage="data.orderInfo.IQ_Rate" />
              <!-- <el-progress :percentage="data.orderInfo.IQ_Rate" :status="data.orderInfo.IQ_Rate >= 100 ? 'success' : 'warning'" /> -->
            </div>
...
import GradeConfirmation from '../../OrderAllocationManagement/_Components/GradeConfirmation.vue'
import RejectOrder from '../../OrderAllocationManagement/_Components/RejectOrder.vue'
...
import OrderTracking from '../../OrderAllocationManagement/_Components/OrderTracking.vue'
import NewPhase from '../../OrderAllocationManagement/_Components/NewPhase.vue'
const store = useCommonStore() //记得加这一句
import { orderTrackingOpen, newPhaseOpen, accountingRecordsOpen, approvalStatusOpen } from '/@/assets/ts/techOpenWindow'
import CreditReport from '../../OrderAllocationManagement/_Components/CreditReport.vue'
import ProcurementReport from '../../OrderAllocationManagement/_Components/ProcurementReport.vue'
