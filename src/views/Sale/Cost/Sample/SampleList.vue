<template>
  <div class="samplelist">
    <!-- 柱状图 -->
    <EchartsMsg :MsgType="4" v-if="EchartsShow"></EchartsMsg>
    <div class="pt-2  ">
      <div class="py-2 text-right flex justify-between">
        ...
        <div class="flex">
          ...
          <el-button type="primary" class="addbutclass" @click="openContract">
            <svg-icon icon-class='svg-addsample' />申请样品
          </el-button>
        </div>
    ...
    <div class="">
      <el-table :data='data.tableData' @selection-change="handleSelectionChange"
        ...
        <el-table-column label='使用时间'>
          <template #default="scope">{{ (scope.row.UsingTime||'').substring(0,11) }}</template>
        </el-table-column>
        <el-table-column label='需求方'>
          <template #default="scope">{{scope.row.NeedName}}</template>
        </el-table-column>
...
import Add from './AddSample.vue'
import EchartsMsg from '/@/views/Sale/Cost/Entertainment/EchartsMsg.vue'
import { GetCostSampleList } from '/@/api/Sale/Cost'
...
const getlist = () => {
  GetCostSampleList(page).then((res) => {
    if (res.code == 1) {
