<template>
  <div class="entertainmentlist">
    <!-- 柱状图 -->
    <EchartsMsg :MsgType="0" v-show="EchartsShow"></EchartsMsg>
    <div class="pt-2">
      <div class="py-2 text-right flex justify-between">
        <div class="pt-1" style="font-size: 16px; color: #666; font-weight: 550">招待费</div>
        <div class="flex">
          ...
          <el-button type="primary" class="addbutclass" @click="openContract"> <svg-icon icon-class="svg-addent" />新增招待费 </el-button>
        </div>
      </div>
    </div>
    <div class="">
      <el-table :data="data.tableData" @selection-change="handleSelectionChange" :header-cell-style="{ background: '#468bfd', color: '#F5F8FF', height: '3rem', textAlign: 'center', fontWeight: '600', fontSize: '14px', padding: '0px' }" :cell-style="{ textAlign: ' center', color: '#333', height: '3rem', padding: '3px' }" style="width: 100%; border-radius: 6px">
        ...
        <el-table-column label="招待人数" width="200">
          <template #default="scope">{{ scope.row.Visitor }}</template>
        </el-table-column>
        <el-table-column label="申请费用">
          <template #default="scope">{{ scope.row.Amount }}</template>
        </el-table-column>
        <el-table-column label="申请理由" min-width="160" show-overflow-tooltip>
          <template #default="scope">{{ scope.row.Note }}</template>
        </el-table-column>
        <el-table-column label="状态">
          <template #default="scope">
            <div class="text-red-600" v-if="scope.row.State < 0">驳回</div>
            <div class="text-yellow-600" v-else-if="scope.row.State == 0">待提交</div>
            <div class="text-blue-500" v-else-if="scope.row.State >= 1 && scope.row.State < 100">审核中</div>
            <div class="text-green-500" v-else-if="scope.row.State == 100">已通过</div>
          </template>
        </el-table-column>
...
import Add from './AddEntertainment.vue'
import EchartsMsg from '/@/views/Sale/Cost/Entertainment/EchartsMsg.vue'
import { GetCostEntertainmentList } from '/@/api/Sale/Cost'
...
//获取列表数据
const getlist = () => {
  GetCostEntertainmentList(page).then((res) => {
    if (res.code == 1) {
