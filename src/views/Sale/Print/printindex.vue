<template>
  <div>
    <shrc v-if="data.viewtype==2" :printdata='printdata' @closeWindow="cancelprintview"></shrc>
...
import bjhzy from './print_bjhzy.vue'
import bffm from './print_bffm.vue'
import bfgzy from './print_bfgzy.vue'
import hg from './print_hg.vue'
import hmzk from './print_hmzk.vue'
import sdlh from './print_sdlh.vue'
import shrc from './print_shrc.vue'
import wfhd from './print_wfhd.vue'
  ...
  if (props.DataId != 0) {
    GetCustomerOrderInfo({ BillId: props.DataId }).then((res) => {
      if (res.code == 1) {
        ...
        res.data.bill.ContactsContact = JSON.parse(
          res.data.bill.ContactsContact
        )
        res.data.bill.SalesmanContact = JSON.parse(
          res.data.bill.SalesmanContact
        )
