<template>
  <div class="contractlist">
    <div class="pt-6">
      ...
      <div class="listtop flex p-2 pl-8 justify-between">
        <div class="flex">
          <div class="rank pt-6 ">
            <div class="text-3xl text-white text-center">
              {{data.Rank.ContractRank}}
            </div>
          </div>
          <div class='bg-blue-bg border-white border-solid border-2 h-16 w-48 rounded-md mt-4 p-2  pl-6 -ml-6'>
            <div class="border-white border-dashed border h-full rounded-md flex text-center justify-between px-2">
              <div>
                <div class="text-xl">{{data.Rank.ContractCount}}</div>
              ...
              <div>
                <div class="text-xl">{{data.Rank.DelContractCount}}</div>
                <div class="text-xs">回收站</div>
              </div>
        ...
        <div class="flex">
          <div class="yjrank pt-6 ">
            <div class="text-3xl text-white text-center">
              {{data.Rank.ContractYJRank}}
            </div>
          </div>
          ...
          <div class='bg-blue-bg border-white border-solid border-2 h-16 w-48 rounded-md mt-4 p-2  pl-6 -ml-6'>
            <div class="border-white border-dashed border h-full rounded-md flex text-center justify-between px-2">
              <div>
                <div class="text-xl">{{data.Rank.SuccessAmount}}</div>
                <div class="text-xs">完成</div>
              </div>
              <div>
                <div class="text-xl">{{data.Rank.UnSuccessAmount}}</div>
                <div class="text-xs">未完成</div>
              </div>
              <div>
                <div class="text-xl">{{data.Rank.SuccessRate}}%</div>
                <div class="text-xs">成单率</div>
              </div>
            </div>
        ...
        <div class="flex">
          <div class="shrank pt-6 ">
            <div class="text-3xl text-white text-center">
              {{data.Rank.AfterSalesRank}}
            </div>
          </div>

          <div class='bg-blue-bg border-white border-solid border-2 h-16 w-36 rounded-md mt-4 p-2 pl-6 -ml-6'>
            <div class="border-white border-dashed border h-full rounded-md flex text-center justify-between px-2">
              <div>
                <div class="text-xl">{{data.Rank.AfterSalesCount}}</div>
                <div class="text-xs">已处理</div>
              </div>
              <div>
                <div class="text-xl">{{data.Rank.UnAfterSalesCount}}</div>
        ...
        <div class="flex">
          <div class="fyrank pt-6 ">
            <div class="text-3xl text-white text-center">
              {{data.Rank.CostRank}}
            </div>
          </div>

          <div class='bg-blue-bg border-white border-solid border-2 h-16 w-40 rounded-md mt-4 p-2  pl-6 -ml-6'>
            <div class="border-white border-dashed border h-full rounded-md flex text-center justify-between px-2">
              <div>
                <div class="text-xl">{{data.Rank.CostAmount}}</div>
                <div class="text-xs">费用支出</div>
              </div>
              <div>
                <div class="text-xl">{{data.Rank.CostRate}}%</div>
                <div class="text-xs">费用占比</div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="text-xs leading-9 text-center">
            <div class="flex ">
              <div class="hjmoney flex">
                ...
                <div class="w-1/2">{{data.Rank.SuccessAmount}}万</div>
              </div>
              <div class="hjmoney flex">
                <div class="w-1/2 text-white">业绩金额合计</div>
                <div class="w-1/2">{{data.Rank.YJAmount}}万</div>
              </div>
            </div>
            <div class="flex">
              <div class="hjmoney flex">
                <div class="w-1/2 text-white">实际金额合计</div>
                <div class="w-1/2">{{data.Rank.RealAmount}}万</div>
              </div>
              <div class="hjmoney flex">
                <div class="w-1/2 text-white">退货金额合计</div>
                <div class="w-1/2">{{data.Rank.ReturnCost}}万</div>
              </div>
            </div>
          </div>
    ...
    <div class="py-1">
      ...
      <div class='hom_custab mt-2  pt-2 flex justify-between  border-gray-400 border-dashed '>
        ...
        <div class="flex pt-1">
          ...
          <el-button type="primary" @click="search" class="butclass mr-2">
            <svg-icon icon-class='svg-cus_search' />搜索
          </el-button>

          <div v-if="data.tableTop==0" class='flex'>

            <el-button type="primary" class="butclass" @click="AddLostOrder">
              <el-icon-List class='w-4 h-4 mr-2' />作废合同
            </el-button>
            <el-button type="primary" class="butclass" @click="openstatus">
              <el-icon-List class='w-4 h-4 mr-2' />状态变更
            </el-button>
            <el-button type="primary" class="addbutclass" @click="openStock">
              <svg-icon icon-class='svg-tbsp' />备货申请
            </el-button>
            <el-button type="primary" class="addbutclass" @click="AddApprovalContract">
              <svg-icon icon-class='svg-tbsp' />提报审批
            </el-button>

            <el-button type="primary" class="addbutclass" @click="AddContractEvent">
              <svg-icon icon-class='svg-addht' />新增合同
            </el-button>
          </div>
          <div v-if="data.tableTop==1" class='flex'>

            <el-button type="primary" class="butclass" @click="AddAfterSales">
              <svg-icon icon-class='svg-addsh' />售后录入
            </el-button>
            <el-button type="primary" class="butclass" @click="Addreturnproducts">
              <svg-icon icon-class='svg-addth' />退货录入
            </el-button>
            <el-button type="primary" class="butclass" @click="AddInvoicing">
              <svg-icon icon-class='svg-addkp' />申请开票
            </el-button>
            <el-button type="primary" class="addbutclass" @click="AddCollectionEvent">
              <svg-icon icon-class='svg-addsk' />新增收款
            </el-button>
          </div>
        </div>
      ...
      <div v-if=" data.tableTop==0">
        <el-table :data='data.tableData' @selection-change="handleSelectionChange" border :header-cell-style="{ 'background':'#468bfd', 'color': '#F5F8FF' ,'height':'4rem', 'textAlign': 'center' ,'fontWeight':'600','fontSize':'14px','padding':'0px' }" :cell-style="{ 'textAlign': ' center','color': '#333', 'height':'3rem','padding':'3px' }" style='width: 100%;border-radius:6px'>
          ...
          <el-table-column label='合同状态' width="100">
            <template #default="scope">
              <div class="text-red-600" v-if="scope.row.BillState<0">
                驳回
              </div>
              <div class="text-yellow-600" v-else-if="scope.row.BillState==0">
                等待提交
              </div>
              <div class="text-blue-500" v-else-if="scope.row.BillState>=1&&scope.row.BillState<90">
                审核中
              </div>
              <div class="text-blue-500" v-else-if="scope.row.BillState>=90&&scope.row.BillState<100">
                {{retstate(scope.row.BillState)}}
              </div>
              <div class="text-green-500" v-else-if="scope.row.BillState==100">
                已通过
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column label='操作' width="100">
            <template #default="scope">
              <el-link type="primary" @click="ViewContractEvent(scope.row)">
                详情
              </el-link>
              <el-link type="primary" v-if="scope.row.BillState<=0" @click="EditContractEvent(scope.row)" class="ml-2">
                编辑
              </el-link>
      ...
      <div v-if=" data.tableTop==1">
        <el-table :data='data.tableData' @selection-change="handleSelectionChange" border :header-cell-style="{ 'background':'#468bfd', 'color': '#F5F8FF' ,'height':'4rem', 'textAlign': 'center' ,'fontWeight':'600','fontSize':'14px','padding':'0px' }" :cell-style="{ 'textAlign': ' center','color': '#333', 'height':'3rem','padding':'3px' }" style='width: 100%;border-radius:6px'>
          ...
          <el-table-column label='操作' width="80">
            <template #default="scope">
              <el-link type="primary" @click="opencontract(scope.row)">
                详情
              </el-link>
            </template>
          </el-table-column>
      ...
      <div v-if=" data.tableTop==2">
        <el-table :data='data.tableData' @selection-change="handleSelectionChange" border :header-cell-style="{ 'background':'#468bfd', 'color': '#F5F8FF' ,'height':'4rem', 'textAlign': 'center' ,'fontWeight':'600','fontSize':'14px','padding':'0px' }" :cell-style="{ 'textAlign': ' center','color': '#333', 'height':'3rem','padding':'3px' }" style='width: 100%;border-radius:6px'>
          ...
          <el-table-column label='操作' width="120">
            <template #default="scope">
              <el-link type="primary" @click="ViewContractEvent(scope.row)" class="mr-2">
                详情
              </el-link>
              <el-link type="primary" @click="Recoverycontract(scope.row.Id)">
                恢复
              </el-link>
            </template>
          </el-table-column>
 ...
 -->
    ...
    <ReturnProducts :title="'退货申请'" v-if="Showreturnproducts.show" :ContractId='Showreturnproducts.contractid' :ViewType='Showreturnproducts.viewtype' :DataId='Showreturnproducts.dataId' @cancelContract="cancelreturnproducts">
    </ReturnProducts>

    <Invoicing :title="'开票申请'" v-if="ShowInvoicing.show" :ViewType='ShowInvoicing.viewtype' :ContractId='ShowInvoicing.contractid' :DataId='ShowInvoicing.dataId' @cancelContract="cancelInvoicing">
    </Invoicing>

    <AddCollection :title="'新增收款'" v-if="ShowCollection.show" :ViewType='ShowCollection.viewtype' :ContractId='ShowCollection.contractid' :DataId='ShowCollection.dataId' @cancelContract="cancelCollection">
    </AddCollection>
    <List v-if="ShowList.show" :ContractId='ShowList.contractid' @cancelContract="CloseListView">
    </List>
    <!-- 备货申请 -->
    <Stockup v-if="StockInfo.show" :state='StockInfo.state' :ViewType='StockInfo.viewtype' :DataId='StockInfo.dataId' @cancelContract="closeStock">
    </Stockup>
    <!-- 状态变更 -->
    <ChangeStatus v-if="StatusInfo.show" :State='StatusInfo.state' :DataId='StatusInfo.dataId' @cancelContract="closestatus">
    </ChangeStatus>
...
import Stockup from '/@/views/Sale/Contract/Stockup/Stockup.vue'
import ChangeStatus from '/@/views/Sale/Contract/Stockup/ChangeStatus.vue'
...
import { GetContractList, GetCancelContractById, GetApprovalContractById, GetContractRankByUserId, GetRecoveryContract } from '/@/api/Sale/Contract'
...
const openstatus = () => {
  ...
  StatusInfo.dataId = multipleSelection.value[0].Id
  StatusInfo.state = multipleSelection.value[0].BillState
...
const openStock = () => {
  ...
  StockInfo.dataId = multipleSelection.value[0].Id
  StockInfo.state = multipleSelection.value[0].BillState
  StockInfo.show = true
}
