<template>
  <div>
    <CusDialog :box-style='boxStyle' :title='title' :is-show='true' @closeWindow='closeWindow'>
      <div class='addcontract text-base'>
          ...
          <el-form ref='ruleFormRef' :model='data' :rules="rules" :inline='true'>
            ...
            <div class="mt-2">
              <div class="flex">
                ...
                <div class="cursor-pointer" :class="tabletopindex==1?'checktabletitle':'tabletitle'" @click="checktop(1)">收货人/联系人</div>
              ...
              <div class="bg-white border-blue-400 border-solid border rounded-b-lg p-2">
                ...
                <div v-show="tabletopindex==1" class="relative ">
                  ...
                  <div class="tableclass">
                    ...
                    <el-table border :data='data.receivinglist' @selection-change="handlereceivingChange" :header-cell-style="{'font-size': '12px',padding:'0px' ,background: '#E7F4FE', color: '#303133',height:'36px', textAlign:'center',fontWeight:'500' }" :cell-style="{ textAlign: 'center',color: '#333',height:'36px',padding:'0px' }" style='width: 100%'>
                      ...
                      <el-table-column type="index" width="100" label='类型'>
                        <template #default="scope">
                          <el-form-item :prop="'receivinglist['+scope.$index+'].ReceivingType'" :rules="rules.ReceivingType">
                            <el-select @change="typechange(scope.$index)" :disabled='data.disabled' v-model="scope.row.ReceivingType" placeholder="请选择类型" class="inputclass">
                              <el-option v-for="item in dic.type" :key="item.DicKey" :label="item.DicValue" :value="item.DicKey"></el-option>
                            </el-select>
                          </el-form-item>
                        </template>
                      </el-table-column>

                      <el-table-column prop='Type' width="100" label='姓名'>
                        <template #default="scope">
                          <el-form-item :prop="'receivinglist['+scope.$index+'].ContactsName'" :rules="rules.ContactsName">

                            <el-select v-if="scope.row.ReceivingType==1" @change="changeContactsName(scope.$index)" allow-create filterable default-first-option :disabled='data.disabled' v-model="scope.row.ContactsName" placeholder="请选择类型" class="inputclass">
                              <el-option v-for="(item,index) in data.SituationData" :key="index" :label="item.Name" :value="item.Name"></el-option>
                            </el-select>
                            <el-input v-else :readonly='data.disabled' v-model="scope.row.ContactsName" placeholder=""></el-input>
                          </el-form-item>
                        </template>
                      </el-table-column>
                      ...
                      <el-table-column prop='Name' width="120" label='联系方式'>
                        <template #default="scope">
                          <el-form-item :prop="'receivinglist['+scope.$index+'].Phone'" :rules="rules.Phone">
                            <el-input :readonly='data.disabled' v-model="scope.row.Phone" placeholder=""></el-input>
                          </el-form-item>
                        </template>
                      </el-table-column>
                      <el-table-column type="index" width="105" label='用途'>
                        <template #default="scope">
                          <el-form-item :prop="'receivinglist['+scope.$index+'].Purpose'" :rules="rules.Purpose">
                            <el-select :disabled='data.disabled' v-model="scope.row.Purpose" placeholder="请选择类型" class="inputclass">
                              <el-option v-for="item in dic.purpose" :key="item.DicKey" :label="item.DicValue" :value="item.DicKey"></el-option>
                            </el-select>
                          </el-form-item>
                        </template>
                      </el-table-column>
                      <el-table-column prop='Duties' label='收货地址'>
                        <template #default="scope">
                          <el-form-item v-if="scope.row.ReceivingType==2" :prop="'receivinglist['+scope.$index+'].Address'" :rules="rules.Address">
                            <el-input :autosize="{ minRows: 1, maxRows: 4 }" type="textarea" :readonly='data.disabled' v-model="scope.row.Address" placeholder=""></el-input>
                          </el-form-item>
                          <span v-else>-</span>
                        </template>
                      </el-table-column>
                      <el-table-column prop='Telephone' width="140" label='物流批次'>
                        <template #default="scope">
                          <el-form-item v-if="scope.row.ReceivingType==2" :prop="'receivinglist['+scope.$index+'].Batch'" :rules="rules.Batch">
                ...
                <div v-show="tabletopindex==3" class="relative">
                  ...
                  <Editor v-model="data.form.ContractTerms" :disabled="data.disabled" apiKey='qagffr3pkuv17a8on1afax661irst1hbr4e6tbv888sz91jc' :init="Editorinit.init">
                  </Editor>
                ...
                <template #default="{ option }">
                  <div class="transfer-label">
                    ...
                    <div class="label-switch" v-if="option.show == 1">
                      <el-switch v-model="option.base" size="large" active-color="#409eff" inactive-color="#e6a23c" inline-prompt active-text="可见" inactive-text="隐藏" :active-value="1" :inactive-value="0">
                      </el-switch>
                    </div>
        ...
        <div v-if="data.ViewType!=4" class="text-center pt-6">
          ...
          <el-button v-if="data.Stock==true" type="warning" @click="openStock()">
            备货申请
          </el-button>
        </div>
        <div v-if="data.Stock==true&&data.ViewType==4">
          <el-button type="warning" @click="openStock()">
            备货申请
          </el-button>
        </div>
      </div>
          ...
          <el-card class='sale_card mt-6' style="padding:0px">
            <div>
              ...
              <div class="text-center pt-6">
                ...
                <el-button type="primary" @click="choicmerge">
                  选择
                </el-button>
    ...
    <!-- 备货申请 -->
    <Stockup v-if="StockInfo.show" :checkId='StockInfo.CheckId' :state='StockInfo.state' :ViewType='StockInfo.viewtype' :DataId='StockInfo.dataId' @cancelContract="closeStock">
    </Stockup>
  </div>
</template>
...
import { GetCustomersByName, GetCustomerInvoiceById, GetAllRegionTree, GetSituationInfoList } from '/@/api/Sale/Customer'
import { GetCompanyByCustomer, GetCustomerOrderList, PostOrderEntityList, GetMergeOrderList } from '/@/api/Sale/CustomerOder'
import { PostAddContract, GetContractInfo, PostApprovalContract } from '/@/api/Sale/Contract'
...
const getDictionaryData = () => {
  //联系人用途
  store.getDict('sale_contract_link_purpose').then((res: any) => {
  ...
  //联系人类型
  store.getDict('sale_contract_link_type').then((res: any) => {
    res.forEach((item: any) => {
      item.DicKey = Number(item.DicKey)
    })
    dic.type = res
...
const typechange = (val: any) => {
  if (data.receivinglist[val].ReceivingType == 2) {
    data.receivinglist[val].Purpose = 2
  }
}

const changeContactsName = (val: any) => {
  var user: any = data.SituationData.find((f: any) => f.Name == data.receivinglist[val].ContactsName)
  if (user) {
    data.receivinglist[val].Phone = user.Telephone
    data.receivinglist[val].Post = user.Duties
  }
}
//#region 客户弹窗
...
const checkcustomer = (val: any) => {
  ...
  SituationInfoList(data.form.CustomerId)
  getcusinvoice()
...
const GetContractInfoEvent = () => {
  ...
  GetContractInfo({ Id: Id }).then((res) => {
    if (res.code == 1) {
      ...
      StockInfo.state = res.data.bill.BillState
...
onMounted(() => {
  ...
  if (props.info.viewType != 1) {
    if (props.info.info.Code == 'Contract_StockUp') {
      StockInfo.CheckId = props.info.CheckId
      StockInfo.viewtype = 3
      data.ViewType = 3
      if (props.info.viewType == 4) {
        StockInfo.viewtype = 4
      }
    } else {
