  ...
  <div>
                    ...
                    <div class="flex justify-end">
                      <div class="flex">
                        <el-form-item label='订单编号:' prop='Ranking'>
                          <div v-if="data.form.OrderNo==''" class="flex text-gray-400 cursor-not-allowed">
                            系统保存生成
                          </div>
                          <div v-else>
                            {{data.form.OrderNo}}
            ...
            <el-card v-show="showdetails==true" class='sale_card mt-4'>
              ...
              <div>
                ...
                <el-form-item label='信用等级' prop="CreditScore">
                  <el-input :readonly='true' @click="openWindowCreditReport" v-model='data.form.CreditScore' placeholder='请输入' class="otherinputclass" />
                </el-form-item>
              ...
              <div v-if="tabletopindex==2">
                ...
                <div class="p-2 overflow-y-auto max-h-80" style="border-radius: 0px;">
                  <div v-for="(item,index) in data.trackList " :key="index" class="border-dashed  border-l-2 border-blue-400 relative p-4">
                    ...
                    <div>
                      {{item.CreatName}}<span class="text-gray-400">{{item.CreateOn}}</span><span class="text-gray-400"> {{item.Stage}}</span>
                    </div>
    ...
    <CusDialog v-if="purchaseview" :box-style='tableboxStyle' title='企业采购原则调查' :is-show='true' @closeWindow='cancelpurchaseview'>
      <el-card class='sale_card '>
        ...
        <div>
          <div>
            客户对以往供应商有哪些要求？客户以往采购的付款方式和竞争对手有哪些？客户对产品质量等级和技术有哪些要求？
          </div>
      ...
      <el-card class="mt-4">
        ...
        <el-row>
          ...
          <el-col :lg="24">
            <el-input :readonly='purchmessage.viewtype==2' placeholder='请输入' type="textarea" v-model="purchmessage.data.Way">
            </el-input>
          </el-col>
        </el-row>
        <div v-if="purchmessage.viewtype==1" class="text-center pt-4">
          <el-button class="cancelbut" @click="cancelpurchaseview">
            取消
          </el-button>
          <el-button type="primary" @click="submitpurch()">
            提交
          </el-button>
        </div>
    ...
    <CusDialog v-if="data.contactview" :box-style='conboxStyle' title='选择联系方式' :is-show='true' @closeWindow='cancelcontact'>
      <el-card class='sale_card'>
        ...
        <div class="text-center pt-4">
          <el-button class="cancelbut" @click="cancelcontact">
            取消
          </el-button>
          <el-button type="primary" @click="changeContact()">
            提交
          </el-button>
        </div>
      </el-card>
    ...
    <CusDialog v-if="data.Inquirystage.show" :box-style='conboxStyle' title='选择询价阶段' :is-show='true' @closeWindow='cancelInquirystage'>
      <el-card class='sale_card'>
        ...
        <div class="text-center pt-4">
          <el-button class="cancelbut" @click="cancelInquirystage">
            取消
          </el-button>
          <el-button type="primary" @click="submitInquirystage()">
            提交
          </el-button>
        </div>
      </el-card>
    ...
    <CusDialog v-if="data.InquiryQuantity.show" :box-style='conboxStyle' title='选择询价数量' :is-show='true' @closeWindow='cancelInquiryQuantity'>
      <el-card class='sale_card'>
        ...
        <div class="text-center pt-4">
          <el-button class="cancelbut" @click="cancelInquiryQuantity">
            取消
          </el-button>
          <el-button type="primary" @click="submitInquiryQuantity()">
            提交
          </el-button>
        </div>
      </el-card>
    ...
    <CusDialog v-if="data.InquiryNature.show" :box-style='conboxStyle' title='选择询价性质' :is-show='true' @closeWindow='cancelInquiryNature'>
      <el-card class='sale_card'>
        ...
        <div class="text-center pt-4">
          <el-button class="cancelbut" @click="cancelInquiryNature">
            取消
          </el-button>
          <el-button type="primary" @click="submitInquiryNature()">
            提交
          </el-button>
        </div>
      </el-card>
...
import CreditReport from '/@/views/Sale/Customer/CreditReport/CreditReport.vue'
...
import { GetCustomersByName, GetSituationInfoList, GetPurchaseList, GetPurchaseInfo, PostAddPurchase, GetCustomerReportsById, GetPaymentInfo, GetCustomer } from '/@/api/Sale/Customer'
import { GetTrackList, PostAddOrderTrack, GetCompanyByCustomer, PostAddCustomerOrder, GetCustomerOrderInfo } from '/@/api/Sale/CustomerOder'
...
import tableindex from './Component/index.vue'
...
const openWindowCreditReport = () => {
  if (data.form.CustomerId == 0) {
    ...
    creditReportOpenData.formData = {
      Id: data.form.CustomerId,
      Name: data.form.CustomerName
    }
...
const closeWindowCreditReport = (type: string) => {
  if (data.form.CustomerId != 0) {
    GetCustomer({ CustomerId: data.form.CustomerId }).then((res) => {
      data.form.CustomerName = res.data.Cus.Name
      data.form.CreditScore = res.data.Cus.CreditScore
    })
  }
