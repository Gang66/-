<template>
    ...
    <div class="py-1">
      <div class="hom_custab mt-2 pt-2 flex justify-between border-gray-400 border-dashed">
        ...
        <div class="flex">
          ...
          <el-button type="primary" class="butclass"> <el-icon-List class="w-4 h-4 mr-2" />恢复失单 </el-button>
          <el-button type="primary" class="addbutclass" @click="openContract"><svg-icon icon-class="svg-addsample" />样品申请</el-button>
    ...
    <div class="">
      <el-table :data="data.tableData" @selection-change="handleSelectionChange" border :header-cell-style="{ background: '#468bfd', color: '#F5F8FF', height: '3rem', textAlign: 'center', fontWeight: '600', fontSize: '14px', padding: '0px' }" :cell-style="{ textAlign: ' center', color: '#333', height: '3rem', padding: '3px' }" style="width: 100%; border-radius: 6px">
        ...
        <el-table-column label="跟单信息" width="110">
          <template #default="scope">
            <span class="text-blue-500 cursor-pointer" @click="ToMessage(scope.row)">查看</span>
          </template>
        </el-table-column>
 ...
 -->
    ...
    <AddSample :title="ShowChildDialog.title" v-if="ShowChildDialog.show" :ViewType="ShowChildDialog.viewtype" :DataId="ShowChildDialog.dataId" @closeWindow="cancelContract"></AddSample>
...
import AddSample from '../Cost/Sample/AddSample.vue'
import printindex from '../Print/printindex.vue'
import { GetCustomerOrderList, GetUpdateLostOrder, GetCustomerOrderRankByUserId } from '/@/api/Sale/CustomerOder'
