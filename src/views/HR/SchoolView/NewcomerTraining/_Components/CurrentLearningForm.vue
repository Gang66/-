<template>
  <div class="CurrentLearningForm">
    <OpenWindow :boxStyle="boxStyle" :isShow="true" @closeWindow="closeWindow('close')" :title="data.planInfo.plan.Name">
      <div class="bf-window-padding" style="padding-top: 6px">
        <div class="flex items-center">
          <tabs :tabList="data.tabList" @handleClick="selectEvent"> </tabs>
        </div>
        <main class="mt-4">
          <div class="flex main-bg relative bg-white rounded items-end mb-3 justify-between" v-for="(item, index) in data.courseList" :class="index + 1 == 4 ? 'mb-0' : ''">
            ...
            <div class="flex">
              ...
              <template v-if="((data.planInfo.plan.IsCourseTest == 1 && item.TestStatus === 100) || (data.planInfo.plan.IsCourseTest == 0 && item.SummaryStatus === 100)) && data.planInfo.plan.IsCourseExam == 1 && item.Exam != null && item.ExamCount > 0">
    ...
    <!-- pdf模板 -->
    <StudyTemplate v-if="data.StudyTemplateWindow.show" @closeWindow="closeWindowTwo" :Id="data.StudyTemplateWindow.Id" :CourseId="data.StudyTemplateWindow.CourseId" :courseList="data.courseList"></StudyTemplate>
    <!--刷题 考试模板 -->
    <TestQuestionsTemplate v-if="data.TestQuestionsTemplateWindow.show" @closeWindow="closeWindowTwo" :details="data.TestQuestionsTemplateWindow.details"></TestQuestionsTemplate>
  </div>
</template>
...
const formEvent = (data: any, closeWindow: any) => {
  ...
  const handleJumpTest = (val: any, value: any, type: any, isBigTest: any) => {
    //isNum 100标识学习完毕 1标识上午未学习 2标识下午未学习
    var isNum: any = 100
    if (isBigTest) {
      data.planInfo.courses.forEach((element: any) => {
        if (element.ExamResult != 1 && element.Stage == 1 && element.ExamCount != 0) {
          isNum = 1
          return
        } else if (element.ExamResult != 1 && element.Stage != 1 && element.ExamCount != 0) {
          isNum = 2
          return
    ...
    if (isNum == 1) {
      ElMessage({
        message: '警告, 请上午小考结束后，进行大考',
        type: 'warning'
      })
      return
    } else if (isNum == 2) {
      ElMessage({
        message: '警告, 请下午小考结束后，进行大考',
        type: 'warning'
      })
      return
    }
    ...
    if (!val.Worked && type != 0) {
      ElMessage({
        message: '警告, 请结束实操后，进行大考',
        type: 'warning'
      })
      return
    }
...
export default defineComponent({
  ...
  setup(props, context: SetupContext) {
    ...
    // 监听页面关闭
    const closeWindowTwo = (type: string) => {
      context.emit('getDteails', type)
      data.TestQuestionsTemplateWindow.show = false
      data.StudyTemplateWindow.show = false
    }
    // 页面样式
