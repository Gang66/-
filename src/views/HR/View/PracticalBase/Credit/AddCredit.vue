  ...
  <div>
        ...
        <div class="salerDialog">
          <el-card class="sale_card">
            ...
            <div>
              ...
              <div class="pt-2">2、通过品牌或专业性或同类使用案例影响采购；通过客情或多次拜访影响采购；通过付款方式或价格影响 采购；通过供应周期和售后服务影响采购。(四选二，并阐述)</div>
    ...
    <CusDialog v-if="messagelog.View" :box-style="mesboxStyle" title="消息记录" :is-show="true" @closeWindow="closemessage">
      <el-card class="sale_card mt-6">
        <el-table border :data="messagelog.tableData" :header-cell-style="{ 'font-size': '12px', padding: '0px', background: '#E7F4FE', color: '#303133', height: '24px', textAlign: 'center', fontWeight: '500' }" :cell-style="{ textAlign: 'center', color: '#333', height: '24px', padding: '0px' }" style="width: 100%">
    ...
    <AddCompetitor v-if="data.AddCompetitor.view" :Data="data.AddCompetitor.infodata" :ViewType="data.AddCompetitor.viewtype" :CustomerId="data.AddCompetitor.customerid" :CustomerName="data.AddCompetitor.cusname" :title="data.AddCompetitor.title" @addCompetitor="addCompetitor" @closeWindow="closeCompetitor" />
  </div>
</template>
<script lang="ts" setup>
import { PostAddCustomerReports, GetCustomerReportsList, GetCustomerReportsById } from '/@/api/hr/School/PracticalBase'
