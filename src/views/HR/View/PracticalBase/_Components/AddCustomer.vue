          ...
          <el-form ref="ruleFormRef" :model="data.ruleForm" :rules="rules">
            <el-card class="sale_card">
              ...
              <div v-if="showdetails == true">
                <div class="bg-white rounded shadow-xl p-1" v-if="data.isShowCustomerMessage == true" style="position: absolute; top: 90px; z-index: 9; left: 130px">该客户已存在，无法提交，可选择“<span class="text-blue-500 cursor-pointer" @click="opentransferv">申请划拨</span>”</div>
                <div class="bg-white rounded shadow-xl p-1" v-if="data.isShowMessage == true" style="position: absolute; top: 90px; z-index: 9; left: 130px">已经是你的客户无需添加</div>
                  ...
                  <div class="flex w-full">
                    ...
                    <div v-if="data.baseInfoDisable == false" class="flex">
                      ...
                      <el-button @click="openRegion" class="rounded-md w-7rem" style="line-height: 0; width: 5rem">
                        <el-icon> <el-icon-location /> </el-icon><br />区域申请
                      </el-button>
                    </div>
                  </div>
                </el-form-item>
                <!-- <el-row v-if="data.ruleForm.Ranking <= 30 && data.ruleForm.Ranking != 0 && data.ruleForm.Id == 0">
                  <el-col :lg="6">
                    <el-form-item label="客户等级" prop="Ranking">
                      <div v-if="data.ruleForm.Ranking > 0" @click="showranking" class="pl-2 w-full border-gray-300 border-solid border rounded" style="color: #db0c0c">{{ data.ruleForm.Ranking }}强</div>
                      <div v-else @click="showranking" class="pl-2 w-full flex text-gray-400 cursor-pointer border-gray-300 border-solid border rounded">请选择</div>
                    </el-form-item>
                  </el-col>
                  <el-col :lg="6">
                ...
                <!-- v-if="data.ruleForm.Ranking > 30 || data.ruleForm.Ranking == 0 || data.ruleForm.Id != 0" -->
                <el-row>
                  <el-col :lg="8">
                    <el-form-item label="客户等级" prop="Ranking">
                      <div v-if="data.ruleForm.Ranking > 0" @click="showranking" class="pl-2 w-full border-gray-300 border-solid border rounded" style="color: #db0c0c">{{ data.ruleForm.Ranking }}强</div>
                      <div v-else @click="showranking" class="pl-2 w-full flex text-gray-400 cursor-pointer border-gray-300 border-solid border rounded">请选择客户等级</div>
                    </el-form-item>
                  </el-col>
                  <el-col :lg="8">
                  ...
                  <el-col :lg="8">
                    <el-form-item label="成立时间" prop="SetupDate">
                      <el-date-picker :disabled="data.baseInfoDisable" v-model="data.ruleForm.SetupDate" type="date" placeholder="请选择" />
                    </el-form-item>
                  </el-col>
                  <el-col :lg="8">
              ...
              <div class="flex justify-between">
                ...
                <div class="mt-1">
                  <div v-if="tabletopindex == 0">
                    <el-button v-if="data.ViewType !== 4" size="small" type="primary" @click="showcontact" style="background: #027aff"> <el-icon-plus class="w-4 h-4 mr-2" /> 新增 </el-button>
                  </div>
                  <div v-if="tabletopindex == 1">
                    <el-button v-if="data.ViewType !== 4" size="small" type="primary" @click="showrelation" style="background: #027aff"> <el-icon-plus class="w-4 h-4 mr-2" /> 新增 </el-button>
                  </div>
                  <div v-if="tabletopindex == 2">
                    <el-button size="small" type="danger" @click="showrelation" style="background: #f38383"> <el-icon-Document class="w-4 h-4 mr-2" /> 编辑 </el-button>
                    <el-button size="small" type="primary" @click="showrelation"> <el-icon-plus class="w-4 h-4 mr-2" /> 历史 </el-button>
                  </div>
                </div>
              </div>
          ...
          <div class="py-4">
            <el-card class="sale_card">
              ...
              <div class="text-center pt-4">
                <el-button class="cancelbut" @click="cancelranking"> 取消 </el-button>
                <el-button @click="CalcCustomerRank"> 提交 </el-button>
              </div>
            </el-card>
          </div>
        </CusDialog>
              ...
              <div>
                <div class="text-center">
                  <a href="http://www.gsxt.gov.cn/index.html" target="_blank">
                    <el-button @click="changeCurrentBtnIndex(0)" :style="checkCustomerNameBtnCurrentIndex == 0 ? 'background:#E6F2FF;color:#333333' : ''"> 国新网 </el-button>
                  </a>
                  <a :href="'https://www.qcc.com/search?key=' + nameForm.name" target="_blank">
                    <el-button @click="changeCurrentBtnIndex(1)" :style="checkCustomerNameBtnCurrentIndex == 1 ? 'background:#E6F2FF;color:#333333' : ''"> 企查查 </el-button>
                  </a>
                  <a :href="'https://www.qixin.com/search?key=' + nameForm.name" target="_blank">
                    <el-button @click="changeCurrentBtnIndex(2)" :style="checkCustomerNameBtnCurrentIndex == 2 ? 'background:#E6F2FF;color:#333333' : ''"> 启信宝 </el-button>
                  </a>
                  <a :href="'https://www.tianyancha.com/search?key=' + nameForm.name" target="_blank">
                    <el-button @click="changeCurrentBtnIndex(3)" :style="checkCustomerNameBtnCurrentIndex == 3 ? 'background:#E6F2FF;color:#333333' : ''"> 天眼查 </el-button>
                  </a>
                  <a :href="'https://aiqicha.baidu.com/s/person?q=' + nameForm.name + '&t=2'" target="_blank">
                    <el-button @click="changeCurrentBtnIndex(4)" :style="checkCustomerNameBtnCurrentIndex == 4 ? 'background:#E6F2FF;color:#333333' : ''"> 爱企查 </el-button>
                  </a>
                </div>
        ...
        <CusDialog v-if="lookintoview" :boxStyle="comboxStyle" title="客户主观调查" :is-show="true" @closeWindow="cancellookinto">
          <div>
            <el-card class="sale_card">
              ...
              <div>
                <div>工资是否准时发放，劳资争议或人员更迭是否频繁？企业管理体系是否完善，员工向心力与企业规模是否匹配？ 企业资金链是否充裕，同行口碑和供应商口碑如何？你认为影响企业信用的还有哪些？</div>
                <div class="py-4">
                  <el-input v-model="data.sitcom.CreditNote" placeholder="请输入" type="textarea" class="w-full"> </el-input>
                </div>
                <div>
                  <div>历史数据：（时间、添加人、内容、信用结果、总计条数）</div>
                ...
                <div class="text-center">
                  <el-button class="cancelbut" @click="cancellookinto"> 取消 </el-button>
                  <el-button @click="PostAddSituationCommEvent"> 提交 </el-button>
                </div>
              </div>
            </el-card>
          </div>
