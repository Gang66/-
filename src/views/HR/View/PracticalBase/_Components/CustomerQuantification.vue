<template>
  <div class="quotationlist">
    <div class="pt-2">
      <div class="border-gray-500 border-dashed p-1 pb-0 flex">
        <div class="w-52">
        ...
        <div class="flex w-full pb-2 justify-end">
          <div class="pr-4">
            <div v-if="data.tableTop == 0" class="flex">
              ...
              <el-button type="primary" class="butclass pr-2" @click="search"> <svg-icon icon-class="svg-cus_search" />搜索 </el-button>
            </div>
            <div v-if="data.tableTop == 1" class="flex">
              ...
              <el-button type="primary" class="butclass" @click="getchangelist"> <svg-icon icon-class="svg-cus_search" />搜索 </el-button>
            </div>
          </div>

          <div v-if="data.tableTop == 0" class="justify-end">
            <!-- <el-button type="primary" class="butclass" @click="openTogether"> <svg-icon icon-class="svg-xiezuo" />邀请协作 </el-button> -->
            <el-button type="primary" class="addbutclass" @click="addCustomerview"> <svg-icon icon-class="svg-add" style="font-size: 18px; margin-right: 3px" />新增客户 </el-button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="">
      <div v-if="data.tableTop == 0">
        <el-table :data="data.tableData" @selection-change="handleSelectionChange" :header-cell-style="{ background: '#468bfd', color: '#F5F8FF', height: '2rem', textAlign: 'center', fontWeight: '600', fontSize: '14px', padding: '0px' }" :cell-style="{ textAlign: ' center', color: '#333', height: '3rem', padding: '3px' }" style="width: 100%; border-radius: 6px">
          ...
          <el-table-column label="客户名称" show-overflow-tooltip width="400">
            <template #default="scope">
              <div class="items-center" style="display: flex; justify-between: center; position: relative">
                ...
                <div class="truncate mr-2 text-left" style="cursor: pointer; color: #409eff; margin-left: 5px" :style="{ maxWidth: scope.row.CompanyName ? '280px' : '380px' }">{{ scope.row.Name }}</div>
          ...
          <el-table-column fixed="right" label="操作" width="80px">
            <template #default="scope">
              <div style="display: flex" class="justify-center">
                <div>
                  <span v-if="scope.row.Checked != 1" class="cursor-pointer" style="color: #fc701b" @click="handleClick(scope.row, 2)"> 未核准 </span>
                  <span v-else class="cursor-pointer" style="color: #44d7b6" @click="handleClick(scope.row, 3)"> 已核准 </span>
                </div>
              </div>
            </template>
          </el-table-column>
      ...
      <div v-if="data.tableTop == 1">
        <el-table :data="data.Changelist" border :header-cell-style="{ background: '#468bfd', color: '#F5F8FF', height: '4rem', textAlign: 'center', fontWeight: '600', fontSize: '14px', padding: '0px' }" :cell-style="{ textAlign: ' center', color: '#333', height: '3rem', padding: '3px' }" style="width: 100%; border-radius: 6px">
          <el-table-column type="selection" width="50" />
          <el-table-column type="index" width="55" label="序号" />
          ...
          <el-table-column label="状态" width="110">
            <template #default="scope">
              <el-link type="danger" v-if="scope.row.State == -2"> 已拒绝 </el-link>
              <el-link type="success" v-else-if="scope.row.State == 100"> 已同意 </el-link>
              <el-link type="primary" v-else> 待确认 </el-link>
            </template>
          </el-table-column>
          <el-table-column fixed="right" label="操作" width="110" style="text-align: left">
            <template #default="scope">
              <el-link @click="showchange(scope.row, 2)" v-if="scope.row.State == -2 || scope.row.State == 100"> 详情 </el-link>
              <el-link type="primary" @click="showchange(scope.row, 4)" v-else> 去确认 </el-link>
            </template>
          </el-table-column>
        </el-table>
        <div class="flex justify-end">
    ...
    <Together v-if="data.AddTogether.view" :ViewType="data.AddTogether.viewtype" :DataId="data.AddTogether.dataid" :CustomerName="data.AddTogether.CusName" :CustomerId="data.AddTogether.CusId" @closeWindow="closeTogether" />
...
import ByCompetingModels from '/@/views/HR/View/PracticalBase/Credit/ByCompetingModels.vue'
import Together from './Together.vue'
// import Communicate from './Component/Communicate.vue'
import Region from './Region.vue'
import { GetCheckedCustomerList, GetCustomerRankByUserId, GetCustomerTogtherList } from '/@/api/hr/School/PracticalBase'
