<template>
  <div class="MaterialForm">
    <OpenWindow :boxStyle="boxStyle" :isShow="true" @closeWindow="closeWindow('close')" title="上传成品">
          ...
          <div>
            <div class="flex justify-between">
              ...
              <div class="flex items-center">
                ...
                <div class="ml-2">
                  <el-switch v-model="data.ruleForm.IsNewest" :active-value="1" :inactive-value="0" :disabled="true" />
                </div>
              </div>
            </div>
            <div class="mt-2">
              <el-form-item label="课件名称" prop="Name">
                <el-input v-model="data.ruleForm.Name" placeholder="请输入" :disabled="true" />
              </el-form-item>
              <div class="flex">
                <el-form-item label="培训需求" prop="TrainingNeeds">
                  <el-select v-model="data.ruleForm.TrainingNeeds" class="input-number" placeholder="请选择" :disabled="true">
                    <el-option :label="item['DicValue']" :value="item['DicKey']" v-for="item in data.TrainingNeeds" />
                  </el-select>
                </el-form-item>
                <el-form-item label="课程等级" prop="Level">
                  <el-select v-model="data.ruleForm.Level" placeholder="请选择" :disabled="true" style="width:200px;">
                ...
                <el-form-item label="课件类型" prop="Class" class="flex-grow">
                  <el-select v-model="data.ruleForm.Class" placeholder="请选择" :disabled="true" class="w-full">
              ...
              <div class="flex w-full">
                <el-form-item label="学习时长" prop="Remark">
                  <el-input-number v-model="data.ruleForm.MinMinutes" :min="1" class="input-number" :disabled="true" />
                </el-form-item>
                <el-form-item label="面向人员" prop="CodeType">
                  <el-select v-model="data.ruleForm.CodeType" multiple collapse-tags collapse-tags-tooltip placeholder="请选择" style="width:200px;" :disabled="data.isdisabled">
                    <el-option :label="item['DicValue']" :value="item['DicKey']" v-for="item in data.CoursewareCodeType" />
                  </el-select>
                </el-form-item>
                <!-- <el-form-item label="面向人员" prop="CodeType">
                  <el-select v-model="data.ruleForm.CodeType" multiple collapse-tags collapse-tags-tooltip class="screen-width" placeholder="请选择" :disabled="true" style="width: 128px">
                ...
                <el-form-item label="课件说明" prop="Contents" class="flex-grow">
                  <el-input v-model="data.ruleForm.Contents" placeholder="请输入" :disabled="true" />
                </el-form-item>
              </div>
            </div>
          </div>
          <div class="mt-4">
            <uploadfile :isdisabled="data.isdisabled" :uploadFileList="data.ruleForm.FileList" @uploadFileData="uploadFileData">
              ...
              <template #rightContent>
                <div>
                  <el-button type="success" @click="$preview(remoteUrlEvent(data.ruleForm.CoverImg))" v-if="data.ruleForm.CoverImg" class="mr-3">查看素材封面</el-button>
                </div>
              </template>
              <template #defaultBtn>
                <el-table-column prop="" label="默认">
                  <template #default="scope">
                    <el-button type="danger" link v-if="data.ruleForm.FileId == scope.row.Id">是</el-button>
                    <el-button type="" link v-else @click="editDefault(scope.row.Id)">否</el-button>
                  </template>
                </el-table-column>
              </template>
            </uploadfile>
...
import { GetAllSysPostList, GetApplyWorkCourseFinishedInfo, GetSetWorkCourseFinishedDefaultSave, GetWorkCourseFinishedEntity, PostEditWorkCourseFinishedSave } from '/@/api/hr/CourseDevelop'
import { parseTime, remoteUrl } from '/@/utils/tools'
import { PostApprovalWorkCourseFinished } from '/@/api/wf/hr/Approval'
const store = useCommonStore() //记得加这一句
const GetContractManagementInfoEvent = async (data: any) => {
  if (data.Id) {
    if (data.viewType == 4 || data.viewType == 5) {
      GetApplyWorkCourseFinishedInfo(data.Id).then((res: any) => {
        if (res.code == 1) {
...
const formEvent = (data: any, closeWindow: any) => {
  ...
  const ApprovalForm = async (formEl: FormInstance | undefined, num: number) => {
    ...
    await formEl.validate((valid, fields) => {
      if (valid) {
        ...
        PostApprovalWorkCourseFinished(arr).then((res: any) => {
          if (res.code == 1) {
            ElMessage({
              message: res.message,
              type: 'success'
  ...
  // 修改默认展示课件
  const editDefault = (Id: any) => {
    GetSetWorkCourseFinishedDefaultSave(data.ruleForm.Id, Id).then((res: any) => {
      if (res.code == 1) {
