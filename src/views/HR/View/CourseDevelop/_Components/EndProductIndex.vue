<!--
 * @Author: ZHJ
 * @Date: 2022-06-17 14:00:56
 * @LastEditTime: 2022-11-23 13:19:30
 * @LastEditors: ZHJ
 * @Description: 
 * @FilePath: \beifangrenzixitong\src\views\HR\View\CourseDevelop\_Components\EndProductIndex.vue
 * 版权声明
-->
<template>
  <table-search :current-page="form.page" :page-size="form.size" :total="form.totals" @size-change="handleSizeChange" :isAdvancedSearch="false" @current-change="handleCurrentChange">
    <div class="bf-table-content travelIndex bg-close-table-checkbox">
      <el-table :data="data.tableData" class="w-full" ref="multipleTableRef" @select="selectRow">
        ...
        <el-table-column prop="FileType" label="课程格式" align="center" />
        ...
        <el-table-column prop="CustomerName" label="学习时长" align="center">
          <template #default="scope">
            <div class="truncate">{{ scope.row.MinMinutes }}分钟</div>
          </template>
        </el-table-column>
        <el-table-column prop="CustomerName" label="题库" align="center">
          <template #default="scope">
            <span class="cursor-pointer">{{ testTotalFilters(scope.row.TestList) > 0 ? '是' : '否' }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="" label="操作" align="center" width="140">
          <template #default="scope">
            <div class="bf-button-bl">
              <el-button text key="primary" type="primary" @click="openEndProductFormWindow(scope.row.Id, 3)">预览</el-button>
              <el-button text key="success" type="success" v-if="scope.row.FileList && scope.row.FileList.length > 0" @click="openEndProductFormWindow(scope.row.Id, 1)">已上传 </el-button>
              <el-button text type="danger" v-else @click="openEndProductFormWindow(scope.row.Id, 1)">未上传</el-button>
              <!-- <el-button text key="danger" type="danger" @click="deleteEvent( scope.row.Id)">下架</el-button> -->
            </div>
          </template>
        </el-table-column>
      </el-table>
...
import EndProductForm from './EndProductForm.vue'
import { defineComponent, onMounted, reactive, ref } from 'vue'
import { useCommonStore } from '/@/store/modules/Common'
import { parseTime } from '/@/utils/tools'
import filterDatetime from '/@/views/HR/Components/filterDatetime/Index.vue'
import { GetEditWorkCourseUndercarriageSave, GetWorkCourseFinishedPage } from '/@/api/hr/CourseDevelop'
...
const window = (form: any, data: any, multipleTableRef: any): any => {
  // 上传素材 弹窗
  const openEndProductFormWindow = async (Id: number, viewType: string) => {
  ...
  const deleteEvent = () => {
    ElMessageBox.confirm('是否下架此条信息?', '提示', {
      ...
      GetEditWorkCourseUndercarriageSave(data.details.Id).then((res) => {
        if (res.code == 1) {
          form.page = 1
...
export default defineComponent({
  ...
  setup() {
    ...
    const form: Isearch = reactive({
      page: 1,
      size: 10,
      IsNewest: 2 //IsNewest==1 新人训  否  IsNewest==2
    })
    const data = reactive({
      ApprovalList: [], //审批状态
      details: {}, //选中详情
