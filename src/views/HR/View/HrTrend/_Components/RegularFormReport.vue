<template>
  <div class="RegularFormView">
    <OpenWindow :boxStyle="boxStyle" :isShow="true" @closeWindow="closeWindow('close')" title="员工转正详情">
      <template #title>
      ...
      <div class="bf-window-padding">
        <div class="mb-6">
          <div class="text-sm">基础、申请信息</div>
          <div class="RegularFormView_hr"></div>
        </div>
        <el-form ref="ruleFormRef" :model="data.ruleForm" :rules="rules" label-width="80px" hide-required-asterisk>
          <div class="flex justify-between">
            ...
            <el-form-item label="性别">
              <el-select v-model="data.ruleForm.Sex" placeholder="请选择性别" class="screen-width" disabled>
          ...
          <div class="flex justify-between items-start">
            ...
            <el-form-item label="备注" class="flex-grow" label-width="140px">
              <el-input v-model="data.ruleForm.Remarks" placeholder="请输入" class="flex-grow" autosize type="textarea" disabled />
            </el-form-item>
          </div>
          <el-form-item label="自我评定总结" class="flex-grow -ml-5" label-width="100px">
            <el-input v-model="data.ruleForm.MyRemarks" placeholder="请输入" class="flex-grow" autosize type="textarea" disabled />
          </el-form-item>
              ...
              <el-tab-pane :label="item.title" :name="item.value" v-for="(item, index) in data.tabList" :key="index" class="px-6">
                <el-table :data="data.probationList" style="width: 100%">
                  ...
                  <el-table-column prop="name">
                    ...
                    <template #default="scope">
                      <el-tooltip class="box-item" effect="dark" placement="top">
                        <template #content> 应出勤{{ scope.row.ShouldAttenDay }}天 实出勤{{ scope.row.OnDuty }}天 出勤率{{ scope.row.AttenRate }}% 出差{{ scope.row.BusinessTravel }}天 迟到{{ scope.row.LateMinute + '/' + scope.row.LateNumber }}(分/次)&nbsp 请假{{ scope.row.LeaveMinute + '/' + scope.row.LeaveNumber }}(分/次)&nbsp 旷工{{ scope.row.AbsenteeismMinute + '/' + scope.row.AbsenteeismNumber }}(分/次) </template>
                        <div class="bf-text-color-primary truncate">应出勤{{ scope.row.ShouldAttenDay }}天 实出勤{{ scope.row.OnDuty }}天 出勤率{{ scope.row.AttenRate }}% 出差{{ scope.row.BusinessTravel }}天 迟到{{ scope.row.LateMinute }}分 请假{{ scope.row.LeaveMinute }}分 旷工{{ scope.row.AbsenteeismMinute }}分</div>
                      </el-tooltip>
                    </template>
                  </el-table-column>
                  <el-table-column prop="" width="260">
                    ...
                    <template #default="scope">
                      <el-tooltip class="box-item" effect="dark" placement="top">
                        <template #content> 合同量{{ scope.row.CheckContractAmount }}个 报价量{{ scope.row.OfferAmount }}万 金额量{{ scope.row.MoneyAmount }}万 渠道量{{ scope.row.ChannelNumber }}个 成交率{{ Math.floor(scope.row.CheckContractNumber / scope.row.OfferNumber) }}% </template>
                        <div class="bf-text-color-primary truncate">合同量{{ scope.row.CheckContractAmount }}个 报价量{{ scope.row.OfferNumber }}万 金额量{{ scope.row.MoneyAmount }}万 渠道量{{ scope.row.ChannelNumber }}个 成交率{{ Math.floor(scope.row.CheckContractNumber / scope.row.OfferNumber) }}%</div>
                      </el-tooltip>
                    </template>
                  </el-table-column>
