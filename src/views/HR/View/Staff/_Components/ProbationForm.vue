<template>
  <OpenWindow :boxStyle="boxStyle" :isShow="true" class="ProbationForm">
    <template #title>
      <div class="flex justify-between items-center py-2 px-3 border-bottom">
        <div class="text-sm">员工试用期报告</div>
        <div class="flex items-center">
          <div class="text-sm bf-text-color-regular mr-4 cursor-pointer" v-for="(item, index) in data.timeData" :key="index" :class="data.selectTimeIndex == index ? 'select_time' : ''" @click="selectTimeIndexEvent(index)">
            {{ item.name }}
          </div>
          <el-select v-model="data.timeValue" placeholder="请选择时间" @change="timeChange" v-if="data.selectTimeIndex == 0">
            <el-option label="全部" :value="0" />
          </el-select>
          <el-select v-model="data.timeValue" placeholder="请选择时间" @change="timeChange" v-else-if="data.selectTimeIndex == 1">
            <el-option v-for="item in data.timeList" :key="item.Id" :label="'第' + item.WeekNumber + '月'" :value="item.Id" />
          </el-select>
          <el-select v-model="data.timeValue" placeholder="请选择时间" @change="timeChange" v-else-if="data.selectTimeIndex == 2">
            <el-option v-for="item in data.timeList" :key="item.Id" :label="'第' + item.WeekNumber + '周'" :value="item.Id" />
          </el-select>
        ...
        <div class="flex justify-between">
          <div class="w-1/2">
            <div class="flex items-center">
              ...
              <div class="trial_report_content_title">考勤得分</div>
          ...
          <div class="w-1/2">
            ...
            <div class="border h-16 mt-2">
              <div class="flex py-3 mx-2">
                <div class="trial_main_section">
                  <div class="trial_main_section_value">{{ data.ruleForm.CheckContractAmount }}/{{ data.ruleForm.SetCheckContractAmount }}</div>
                  <div class="trial_main_section_label">合同量(万)</div>
                </div>
                <div class="trial_main_section">
                  <div class="trial_main_section_value">{{ data.ruleForm.OfferAmount }}/{{ data.ruleForm.SetOfferAmount }}</div>
...
const GetContractManagementInfoEvent = async (data: any) => {
  if (data.userId) {
    GetHRTrialDetailInfo(data.userId)
      .then((res: any) => {
        if (res.code == 1) {
          data.list = res.data
          res.data.forEach((element: any) => {
            if (element.DateType == 0) {
              data.ruleForm = element.infolist[0]
              data.timeValue = element.DateType
            }
          })
        } else {
...
const formEvent = (data: any, closeWindow: any) => {
  ...
  // 监听tab下标
  const selectTimeIndexEvent = (index: number) => {
    data.list.forEach((element: any) => {
      if (element.DateType == index) {
        if (index == 0) {
          data.timeValue = 0
        } else {
          data.timeValue = element.infolist[0].Id
        }
        data.ruleForm = element.infolist[0]
        data.timeList = element.infolist
      }
    })
    data.selectTimeIndex = index
  }
  const timeChange = (val: any) => {
    if (data.timeList && data.timeList.length > 0) {
      data.timeList.forEach((element: any) => {
        if (val == element.Id) {
          data.ruleForm = element
        }
      })
    }
    data.timeValue = val
