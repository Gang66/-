<template>
  <div class='flex rounded items-center top-time-content filterDatetime'>
    <div v-for='(item, index) in data.list' :key='index' class='time bf-text-color-regular cursor-pointer'
      :class='[item.type == &apos;whole&apos; ? &apos;rounded-l&apos; : &apos;&apos;, item.type == &apos;year&apos; ? &apos;rounded-r&apos; : &apos;&apos;, data.selectIndex == index ? &apos;selectBg &apos; : &apos;&apos;]'
      @click='selectEvent(index, item.type)'>{{ item.label }}</div>
