<template>
  <el-icon :size='32' class='text-2xl px-2 py-1 cursor-pointer rounded-l-md' @click='drawer=!drawer'>
    <el-icon-tools />
  </el-icon>
    ...
    <div class='pt-4'>
      ...
      <el-row :gutter='20'>
          ...
          <div class='flex shadow-lg border border-gray-100 w-18 cursor-pointer m-1' @click='changemenubarMode("vertical")'>
            ...
            <div class='flex flex-col flex-1'>
              ...
              <div class='flex-1 relative' :style='{"backgroundColor": theme[getSetting.theme].mainBg}'>
                <el-icon v-if='getSetting.mode==="vertical"' class='absolute left-2/4 top-2/4 transform -translate-x-2/4 -translate-y-2/4'
        ...
        <el-col :span='8'>
          <div class='flex flex-col shadow-lg border border-gray-100 w-18 cursor-pointer m-1 h-12' @click='changemenubarMode("horizontal")'>
            ...
            <div class='flex flex-col flex-1'>
              <div class='flex-1 relative' :style='{"backgroundColor": theme[getSetting.theme].mainBg}'>
                <el-icon v-if='getSetting.mode==="horizontal"' class='absolute left-2/4 top-2/4 transform -translate-x-2/4 -translate-y-2/4'
                  :style='{"color": getSetting.color.primary}'>
                  <el-icon-check />
                </el-icon>
              </div>
      ...
      <div class='mt-5 text-sm'>
        <div class='flex justify-between items-center py-1'>
          <div>开启右侧功能模块</div>
          <el-switch v-model='showLayoutRight' />
        </div>
        <div class='flex justify-between items-center py-1'>
        ...
        <div>
          ...
          <div class="pt-4">
            <el-row :gutter='20'>
              <el-col :span='8'>
                <div class="w-10 h-10 cursor-pointer" @click='changeSalerDialog(0)'
                  :style="getSetting.salerDialogType==0?'background:#76D2FF;border: 2px solid #3C59D6;':'background:#76D2FF'"></div>
              </el-col>
              <el-col :span='8'>
                <div class="w-10 h-10 cursor-pointer" @click='changeSalerDialog(1)'
                  :style="getSetting.salerDialogType==1?'background:#FEB46B;border: 2px solid #3C59D6;':'background:#FEB46B'"></div>
              </el-col>
              <el-col :span='8'>
                <div class="w-10 h-10 cursor-pointer" @click='changeSalerDialog(2)'
                  :style="getSetting.salerDialogType==2?'background:#AFB1FF;border: 2px solid #3C59D6;':'background:#AFB1FF'"></div>
              </el-col>
            </el-row>
...
export default defineComponent({
  ...
  setup() {
    const {
      ...
      changeSalerDialog,
      changeTagsSetting,
      changePinSearchSetting,
      changeLayoutRightSetting,
      changemenubarMode
    } = useLayoutStore()
    const drawer = ref(false)
    const showTags = ref(getSetting.showTags)
    const showLayoutRight = ref(getSetting.showLayoutRight)
    ...
    watch(
      () => showLayoutRight.value,
      () => changeLayoutRightSetting(showLayoutRight.value)
    )
