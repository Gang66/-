...
import { login, loginParam, getRouterList, getUser, getNewToken, ILoginData, PostGetQRCodeLoginInfo } from '/@/api/layout/index'
...
const { ACCESS_TOKEN, REFRESH_TOKEN } = getLocal<IStatus>('token')
...
export const useLayoutStore = defineStore({
    ...
    state: ():ILayout => ({
        ...
        userInfo: {
            ...
            Subject: null,
            TesttingTime: null,
            TrainStudyStatus: null,
            UnWorkStudyDate: null,
            UnWorkStudyEnableStatus: 0,
            UnWorkStudyStatus: null,
            UpdateBy: null,
            UpdateOn: null,
            UserName: "0",
            WorkIntegral: null,
        ...
        setting: {
            ...
            salerDialogType:setting.salerDialogType !== undefined ? setting.salerDialogType : 0,
            showTags: setting.showTags !== undefined ? setting.showTags : true,
            showLayoutRight: setting.showLayoutRight !== undefined ? setting.showLayoutRight : true,
            color: {
    ...
    actions: {
        ...
        // 修改销售弹窗主题
        changeSalerDialog(num?:number):void {
            if(num === this.setting.salerDialogType) return
            if(typeof num !== 'number') num = this.setting.salerDialogType
            this.setting.salerDialogType = num
            localStorage.setItem('setting', JSON.stringify(this.setting))
        },
        // 修改主题色
        ...
        // 修改右侧快捷菜单
        changeLayoutRightSetting(showLayoutRight:boolean):void{
            this.setting.showLayoutRight = showLayoutRight
            localStorage.setItem('setting', JSON.stringify(this.setting))
        },
        ...
        async login(param: loginParam):Promise<IResponse<ILoginData>> {
            const res = await login(param)
            if(res.code === 1 && res.data) {
                const { AccessToken, RefreshToken } = res.data
                this.status.ACCESS_TOKEN = AccessToken
                this.status.REFRESH_TOKEN = RefreshToken
                setLocal('token', this.status, 1000 * 60 * 60)
        ...
        async getNewToken():Promise<void> {
            const info = {
                AccessToken: this.status.ACCESS_TOKEN,
                RefreshToken: this.status.REFRESH_TOKEN
            }
            await getNewToken(info).then(res => {
                if(res.code === 1) {
                    const { AccessToken, RefreshToken } = res.data
        ...
        async getUser():Promise<void> {
            ...
            if(res && res.code === 1) {
                ...
                // this.userInfo.role = [userInfo.UserName]
